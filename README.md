...
2. Players connect their IOTA wallet and contribute (minimum ~1 USD in IOTA)
3. Each player gets 1 draw ticket — equal chance regardless of contribution size
4. Players can optionally enter a "lucky phrase" — mixed into the randomness seed
...
│   ├── tests/
│   │   └── matka_pot_tests.move       12 unit tests (all passing)
...
| Minimum entry | ~1 USD in IOTA (live price) | Prevents dust spam while keeping it accessible |
...
A: They are hashed (SHA3-256 rolling accumulation) and stored in `accumulated_entropy` on-chain. Individual phrases are visible in `TicketIssued` events. The hash is mixed with `iota::random` at reveal for the final seed.
