...
export default function GameDetailPage() {
  ...
  // Use the game's actual on-chain minimum, not a USD-derived approximation.
  const gameMinIOTA = Number(game.minContributionNanos) / 1e9;

  const playerCount = game.players.length;
  const alreadyIn = !!(account && game.players.includes(account.address));
  const entryCount = playerCount + (alreadyIn ? 0 : 1);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(amtIOTA) || amtIOTA < gameMinIOTA) return;
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="text-gray-400 text-sm block mb-1">
                      Amount (IOTA) <span className="text-gray-600">· min {gameMinIOTA.toFixed(2)} IOTA</span>
                    </label>
                    <input
                      type="number"
                      step="0.01"
                      min={gameMinIOTA.toFixed(2)}
                      value={amount}
                      onChange={e => setAmount(e.target.value)}
                      placeholder={gameMinIOTA.toFixed(2)}
                    ...
                    {amount && parseFloat(amount) < gameMinIOTA && (
                      <p className="text-red-400 text-xs mt-1">
                        Minimum entry is {gameMinIOTA.toFixed(2)} IOTA
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={isPending || !amount || parseFloat(amount) < gameMinIOTA}
